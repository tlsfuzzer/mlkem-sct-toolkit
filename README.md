...
```
openssl genpkey -algorithm ml-kem-512 -out ml-kem-512-dk.pem
openssl pkey -pubout -in ml-kem-512-dk.pem -out ml-kem-512-ek.pem
openssl genpkey -algorithm ml-kem-768 -out ml-kem-768-dk.pem
openssl pkey -pubout -in ml-kem-768-dk.pem -out ml-kem-768-ek.pem
openssl genpkey -algorithm ml-kem-1024 -out ml-kem-1024-dk.pem
openssl pkey -pubout -in ml-kem-1024-dk.pem -out ml-kem-1024-ek.pem
```
...
The CPU 0 here is assumed to be one of the _isolated_ cores, see [tlsfuzzer documentation](https://tlsfuzzer.readthedocs.io/en/latest/timing-analysis.html#environment-setup).

### ML-KEM-512
```
PYTHONPATH=../tlsfuzzer taskset --cpu-list 0 ../tlsfuzzer/venv-py3-opt-deps/bin/python3 harness/kyber-py/mlkem_decap.py -i test-dir/ciphers.bin -o test-dir/raw_times.csv -k ml-kem-512-dk.pem -n 768
```
### ML-KEM-768
```
...
### ML-KEM-1024
```
PYTHONPATH=../tlsfuzzer taskset --cpu-list 0 ../tlsfuzzer/venv-py3-opt-deps/bin/python3 harness/kyber-py/mlkem_decap.py -i test-dir/ciphers.bin -o test-dir/raw_times.csv -k ml-kem-1024-dk.pem -n 1568
```
